      ...
      for ( ; ; ) {
          ...
          if( (pid = fork()) == 0) {
              ...
              while(fgets(buf, sizeof(buf), stdin)){
                  ...
                  //encerra o servidor
                  if(strcmp(buf, "quit\n") == 0){
                     if(shutdown (listenfd, 2)!=0){
                       //falha ao enerrar servidor avisar cliente
                       printf("Erro, falha ao encerrar a conexão TCP.\n");
                     }
                    //FONTE: http://www.gnu.org/software/libc/manual/html_node/Closing-a-Socket.html
                  }
